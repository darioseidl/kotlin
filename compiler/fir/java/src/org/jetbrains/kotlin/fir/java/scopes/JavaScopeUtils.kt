...
object BuiltinMethodsWithSpecialGenericSignature {
    ...
    @JvmStatic
    fun getDefaultValueForOverriddenBuiltinFunction(
        functionSymbol: FirNamedFunctionSymbol,
        containingScope: FirTypeScope
    ): SpecialGenericSignatures.TypeSafeBarrierDescription? {
        if (functionSymbol.name !in ERASED_VALUE_PARAMETERS_SHORT_NAMES) return null
        return functionSymbol.firstOverriddenFunction(containingScope) {
            it.fir.computeJvmSignature() in SIGNATURE_TO_DEFAULT_VALUES_MAP.keys
        }?.let { SIGNATURE_TO_DEFAULT_VALUES_MAP[it.fir.computeJvmSignature()] }
    }
    ...
    fun FirNamedFunctionSymbol.isBuiltinWithSpecialDescriptorInJvm(containingScope: FirTypeScope, session: FirSession): Boolean {
        ...
        return getSpecialSignatureInfo(containingScope)?.isObjectReplacedWithTypeParameter ?: false || doesOverrideBuiltinWithDifferentJvmName(containingScope, session)
    }
...
object ClassicBuiltinSpecialProperties {
    ...
    fun hasBuiltinSpecialPropertyFqName(symbol: FirCallableSymbol<*>, containingScope: FirTypeScope): Boolean {
        return findBuiltinSpecialPropertyFqName(symbol, containingScope) != null
    }
